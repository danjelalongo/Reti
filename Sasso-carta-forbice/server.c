...
int main(int argc, char*argv[]) {
    ...
    int sockfd, n, newsock, c1, c2, c1_lives, c2_lives, num_reg=0;
    ...
    while(c1_lives > 0 && c2_lives > 0) {
        ...
        if((n = recv(c1, c1_buff, BUFSIZ, 0)) < 0)handle_error("recv");
        c1_buff[n] = 0;
        if((n = recv(c2, c2_buff, BUFSIZ, 0)) < 0)handle_error("recv");
        c2_buff[n] = 0;
